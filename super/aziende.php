...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    header('Location: /super/aziende.php');
    exit;
}

// Handle restore
if (isset($_GET['restore']) && is_numeric($_GET['restore'])) {
    $pdo->prepare("UPDATE azienda SET valida = 1 WHERE idA = ?")->execute([$_GET['restore']]);
    set_flash('success', 'Azienda riattivata.');
    header('Location: /super/aziende.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check unique P.IVA
    $checkStmt = $pdo->prepare("SELECT idA FROM azienda WHERE p_iva = ? AND idA != ?");
    $checkStmt->execute([$p_iva, $idA ?? 0]);
    if ($checkStmt->fetch()) {
        set_flash('error', 'Partita IVA già registrata.');
        header('Location: /super/aziende.php' . ($idA ? "?edit=$idA" : '?new=1'));
        exit;
    }
...
<div class="container">
    <div class="page-header">
        ...
        <a href="/super/aziende.php?new=1" class="btn btn-primary">+ Nuova Azienda</a>
    </div>
    ...
    <div class="form-card mb-3">
        ...
        <form method="POST">
            ...
            <div class="d-flex gap-1">
                ...
                <a href="/super/aziende.php" class="btn btn-secondary">Annulla</a>
            </div>
        </form>
    </div>
    <?php endif; ?>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= h($az['p_iva']) ?></td>
                    <td><?= h($az['nome_riferimento_interno'] ?? '—') ?></td>
                    <td><?= h($az['email'] ?? '—') ?></td>
                    <td><?= h($az['telefono'] ?? '—') ?></td>
                    <td><?= $az['valida'] ? '<span class="badge badge-success">Attiva</span>' : '<span class="badge badge-danger">Disattivata</span>' ?></td>
                    <td>
                        <div class="d-flex gap-1">
                            <a href="/super/aziende.php?edit=<?= $az['idA'] ?>" class="btn btn-sm btn-secondary">Modifica</a>
                            <?php if ($az['valida']): ?>
                            <a href="/super/aziende.php?delete=<?= $az['idA'] ?>" class="btn btn-sm btn-danger"
                               onclick="return confirm('Disattivare questa azienda?')">Disattiva</a>
                            <?php else: ?>
                            <a href="/super/aziende.php?restore=<?= $az['idA'] ?>" class="btn btn-sm btn-success">Riattiva</a>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
                ...
                <tr><td colspan="8" class="text-center" style="padding:2rem;color:var(--gray-400)">Nessuna azienda registrata.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
